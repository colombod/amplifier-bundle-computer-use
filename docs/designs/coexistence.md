...
### 5.7 Remote transport staleness — a measured gap, declared, not fixed by inventing a number

§5.5 states the Windows cost table on the assumption that `idle_source()` is an in-process call. Over `RemoteBackend` (`docs/designs/remote-transport.md` §5) it is not: `presence_idle_ms()` is an SSH round trip — on Windows, a fresh `powershell.exe` spawn per read. Measured directly (`windows-host`, n=80, `key("shift")`, 80/80 registration):

```
min=296.0  p50=781.0  p90=828.0  p95=843.0  p99=875.0  max=875.0  mean=779.6
```

Reasoning about that observation with `windows-wsl2`'s 20ms `guard_ms` alone reasons about data that is up to ~40x staler than the band implies — presenting a locally-measured number as if it applied end to end, exactly the "claims a guarantee it does not have" failure §5.5 already refuses for Windows intra-`type_text` detection. It was undeclared, which is the part that violated this document's own standard.

**What closes, mechanically, and what does not:**

1. **A real bug, fixed.** `PresenceMonitor.sample()` captured its `now` anchor *before* issuing the `idle_source()` call, then compared it against an `idle_ms` that is only valid as of roughly the *end* of that call. For an in-process call (µs) this is invisible. For a remote call (up to 875ms) it manufactures additional staleness on top of what the transport already costs, biased in the permissive direction (`remote_backend.py`'s own docstring: "makes a human look older than they are" — a false negative in a safety gate). The fix: capture `now` immediately *after* `idle_source()` returns. Explicit `now=` (every unit test, on a fake clock) is untouched — those tests already control both instants themselves.
2. **What is NOT invented.** No fixed "remote guard band" constant was added, and `GUARD_MS` itself is untouched — those values stay exactly what they say: per-platform, *locally* measured constants with recorded provenance (§5.5, §9.6). A single hardcoded remote figure (e.g. "900ms for all remote sessions") would apply one box's network conditions to every deployment, which is the same category of dishonesty this fix closes, just relocated.
3. **What is declared instead.** `PresenceMonitor.sample()` now measures the actual wall-clock cost of its own `idle_source()` call, every time, and reports it as `transport_latency_ms` on the returned `PresenceSnapshot` (~0 for an in-process read, real and large for a remote one) — never a fixed or looked-up value, so it tracks whatever the live connection is actually doing. `effective_staleness_ms = guard_ms + transport_latency_ms` is the single honest number for "how wide is the window in which a human touch on this backend could go undetected by this sample." Surfaced in `PresenceSnapshot.to_dict()`, in `HaltedError`'s own message when transport cost is non-trivial, in `ComputerTool.halt_notices`, and as a one-time declaration logged when `_build_coexistence_guard` mounts a guard for a backend with `is_remote = True`.
4. **What remains unproven.** The ordering fix removes the self-inflicted bias (a full round trip's worth); it does not — and, without instrumenting the remote side to report its own read timestamp, cannot — remove the residual "response transit" tail between the target capturing `idle_ms` and the reply reaching the controller. `transport_latency_ms` reports the conservative upper bound (the full measured round trip), never a smaller estimate of that residual. This has not been re-measured on real remote hardware as part of this fix (no remote target was touched); the 296-875ms figure above is cited as the evidence that motivated the fix, not re-derived by it.
5. **Why the threshold itself is not widened.** A tempting-looking alternative is comparing the (still RTT-biased) `margin_ms` against `guard_ms + transport_latency_ms` instead of `guard_ms` alone. That makes detection *less* sensitive, not more — the bias already pushes `margin_ms` down by roughly the transport latency, so widening the comparison threshold in the same direction stacks two suppressive effects into one and makes the false-negative window *larger*. The one sound classification-side correction (subtracting the read's own latency back out of the threshold) depends on the write side's transit-time error being negligible, which is unverified over links with this much jitter (296-875ms spread) and cannot be validated here. Reporting the honest number and fixing the mechanical bias is the move this codebase's own "declare, don't fake" standard supports; inventing a compensating formula this pass could not verify is not.
